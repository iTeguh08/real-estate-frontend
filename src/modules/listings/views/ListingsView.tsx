...
import { formatCount, formatPerSqftPrice } from '@/lib/format-property';
...
export function ListingsView({
  ...
  featuredProperties: initialFeatured,
  bestValueProperties: initialBestValue,
}: ListingsViewProps = {}) {
  ...
  const filters = useMemo(
    () => normalizeListingsFilters(new URLSearchParams(searchKey)),
    [searchKey]
  );

  useHydrateQueryCache(() => {
    if (initialFilters && initialSearchResult) {
      queryClient.setQueryData(
        queryKeys.properties.search(listingFiltersQueryVars(initialFilters)),
        initialSearchResult
      );
    }
    if (initialFeatured) {
      queryClient.setQueryData(queryKeys.properties.featured(), initialFeatured);
    }
    if (initialBestValue) {
      queryClient.setQueryData(queryKeys.properties.bestValue(), initialBestValue);
    }
  });
