...
export default function CookiesRoute({ page, brand }: CookiesPageProps) {
  const queryClient = useQueryClient();
  useHydrateQueryCache(() => {
    queryClient.setQueryData(queryKeys.pages.cookies(), page);
  });

  const title = `${page.title} | ${brand}`;
  const description = page.copy.replace(/<[^>]+>/g, '').slice(0, 160);
  const canonical = absoluteUrl(routes.cookies) || routes.cookies;
